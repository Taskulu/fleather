...
import 'package:parchment/src/document/attributes.dart';
...
class UnsetMentionRule extends DeleteRule {
  ...
  bool _isMention(Map<String, dynamic> attr) =>
      attr.containsKey(ParchmentAttribute.mention.key);

  bool _isPartOfSameMention(Map<String, dynamic> a, Map<String, dynamic> b) =>
      _isMention(a) &&
      _isMention(b) &&
      (a[ParchmentAttribute.mention.key] == b[ParchmentAttribute.mention.key]);
  ...
  Delta? apply(Delta document, int index, int length) {
    ...
    if (partOfMentionIsBefore) {
      delta = delta
        ...
        ..retain(previousText.length,
            previousAttrs..[ParchmentAttribute.mention.key] = null);
    } else {
    ...
    if (partOfMentionIsAfter) {
      delta = delta
        ..retain(nextText.length,
            nextAttrs..[ParchmentAttribute.mention.key] = null);
    }
